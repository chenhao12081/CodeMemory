...
function calculateMetrics(results: EvalResult[]) {
    ...
    return {
        ...
        macroF1: byIntent.reduce((sum, metric) => sum + metric.rawF1, 0) / byIntent.length,
        missingIntents: byIntent.filter((metric) => metric.samples === 0).map((metric) => metric.intent),
        byIntent,
    };
}

function printConfusionMatrix(results: EvalResult[]) {
...
async function main() {
    ...
    console.log("\n按意图统计");
    console.table(metrics.byIntent.map(({ rawF1, ...metric }) => metric));
